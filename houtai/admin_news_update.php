...
<div class="main">
<p class="bian">修改新闻</p>
    <form method="post" action="admin_news_update_ok.php">
    标题<input type="text" name="name" value="<?php echo $res['title']?>" /><br />
    ...
    ?>
     ...
     <option value="<?php echo $res1['id'].'|0|0';?>"
        <?php
        if ($lmname==$res1['lm1']) {
            echo "selected";
        }
        ?>
 ><?php echo $res1['lm1'].'(一级)';?></option>
        <?php
        ...
        {
        if ($lmname==$res2['lm2']) {
             echo "<option value='$res1[id]|$res2[id]|0' selected >------$res2[lm2](二级)</option>";
        }else{
        ...
        while($res3=mysql_fetch_assoc($query3)) {
             if ($lmname==$res3['lm3']) {
             echo "<option value='$res1[id]|$res2[id]|$res3[id]' selected>******$res3[lm3](三级)</option>";
        }else{
...
?>  
    ...
    作者<input type="text" name="user" value="<?php echo $res[adduser];?>" /><br />
新闻内容
        <textarea name="content1" style="width:700px;height:200px;visibility:hidden;">
        <?php echo $res[content];?></textarea>
		<br />
